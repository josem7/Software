class JoinClasesBuscoController < ApplicationController
  ...
  def unirme_cursos
    @join_evento = JoinClasesBusco.new(uid: current_user.id , eid: @evento.id)
  ...
  def abandonar_cursos
    JoinClasesBusco.where(uid: current_user.id , eid: @evento.id).destroy_all
  ...
  def set_evento
    @evento = ClasesBusco.find(params[:id])
  end
