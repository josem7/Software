class JoinClasesOfrezcoController < ApplicationController
  ...
  def unirme_cursos
    @join_evento = JoinClasesOfrezco.new(uid: current_user.id , eid: @evento.id)
  ...
  def abandonar_cursos
    JoinClasesOfrezco.where(uid: current_user.id , eid: @evento.id).destroy_all
  ...
  def set_evento
    @evento = ClasesOfrezco.find(params[:id])
  end
